...
<img src="/img/gsac18_semi_final_board.png" width="560"/>
...
最终选手提交的是一个`<实例Id, 机器Id>`的列表，即部署/迁移动作列表。但并 **不是** 简单地给出实例及其最终部署的目标机器（集群的 **终态**），而是从 **初态** 到选手得到的 **终态** 的全部过渡动作，而且每步动作都要满足资源容量约束和应用干扰约束。这相当于[二分图](https://en.wikipedia.org/wiki/Bipartite_graph)的匹配问题。好在比赛不限制动作的总数，所以不必是最优匹配。
...
> 离线部署功能（或称短期规划）也可以从 Sigma 中划分出来，作为单独的组件，但两者有不少功能是可以共用的（比如约束检查），所以我认为合在一起更合适些。
...
- 分时：对CPU，标准差 StdVar < 0.5 的 共有 46048 个（~68%）实例，分别属于 8613 个（~92%）应用； 对内存，StdVar < 0.5 的 共有 50095 个（~73%）实例，涉及 7283 个（~78%）应用
...
一个作业内任务之间存在 **执行顺序约束**。不同作业间没有这个约束。
...
另一方面，上面也提到，可以转为 **二分图匹配** 问题，从 **终态和初态的差异** 生成比较短的迁移动作。要从差异生成迁移动作，要有足够的空闲资源（机器）进行腾挪，否则可能无法在满足资源和干扰约束的情况下到达终态。
